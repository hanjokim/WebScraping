...
if __name__ == "__main__":
    ...
    for reg in regions:    
        print("\n>>> Ready to update " + reg + ", continue?", end=' ')
        cont = input("[Y/n]")
        if re.match(r'[^y|Y|\n]', cont):
            break
    ...
    #    driver = initPage(headless)
        ...
        while not endOfPageList: # 각 지역 변호사 목록 페이지의 끝
            target_URL = generateURL(reg, page)
            ...
            if update_complete: # 업데이트 완료시 새 목록 뒤에 이전 목록을 붙이고 종료
                ...
                print(">>> %s region list update completed: %d item(s) added" % (reg, update_number))
                break
